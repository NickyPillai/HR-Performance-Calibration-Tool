...
export function EmployeeTable() {
  ...
  const handleAddEmployee = useCallback(async () => {
    await addEmployee();
    // Scroll to keep the table in view after adding
    ...
    return (
      <div className={clsx(
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <p className={isDark ? 'text-slate-300' : 'text-gray-600'}>Loading employee data...</p>
        </div>
      </div>
    );
  }
  ...
  if (employees.length === 0) {
    return (
      <div className={clsx(
        ...
        <p className={clsx('mb-3', isDark ? 'text-slate-400' : 'text-gray-500')}>
          No employees loaded. Import data using the file uploader above or add employees manually.
        </p>
        <button
          onClick={handleAddEmployee}
          className="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-medium rounded-lg transition-colors shadow-lg inline-flex items-center gap-2"
        >
  ...
  return (
    <div ref={tableRef} className={clsx(
      ...
      <div className={clsx(
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleAddEmployee}
              className="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-medium rounded-lg transition-colors shadow-lg flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
